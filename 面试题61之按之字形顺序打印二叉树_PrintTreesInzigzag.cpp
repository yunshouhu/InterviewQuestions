 ...
 * (Question 59 in <Coding Intervies>) How do you print a binary tree by level in zigzag order, 
 * each level in a line? That is to say, nodes on the first level are printed from left to right, 
 * nodes on the second level are printed from right to left, nodes on the third level are printed 
